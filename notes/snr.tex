...
Here we describe how to calculate the PTA SNR for a set of pulsars and a predicted gravitational wave background signal. 
We follow the method in~\cite{SiemensEtAl:2013}. 

The power spectrum for a stochastic gravitatoinal wave background is given by
\begin{equation}
...
Throughout this work we assume $a=-2/3$ consistent with a stochastic background from circular massive black hole binaries and $A = 2 \times 10^{-15}$ consistent with recent possible detection hints~\cite{NANOGrav12.5GWB:2020}.

For a gravitational wave background signal and a given pulsar timing array with total timespan $T$, the average SNR is given by
\begin{equation}
...
The Hellings and Downs coefficients $\chi_{IJ}$ describe the expected angular correlation produced by the gravitational wave signal in pairs of pulsars and is given by % [ref]
\begin{eqnarray}
\chiIJhd &=& \frac{1}{2} 
...
Pulsars may also contain spin noise (or red noise) and jitter noise which is due to changes in the pulse profile. 
